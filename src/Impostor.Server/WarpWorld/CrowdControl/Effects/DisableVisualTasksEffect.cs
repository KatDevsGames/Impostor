...
{
    /// <summary>
    /// An implementation of <see cref="Effect"/> which disables Visual Tasks
    /// </summary>
    public class DisableVisualTasksEffect : Effect
    {
        public override string Code => "DisableVisualTasks";
        ...
        internal DisableVisualTasksEffect(IGame game, GameManager gameManager, IGameStateTracker gameStateTracker, IExternalCommandFactory externalCommandFactory)
            : this(game, gameManager, gameStateTracker, externalCommandFactory, Serilog.Log.ForContext<DisableVisualTasksEffect>())
        {

        }

        /// <summary>
        /// Internal ctor for creating a <see cref="DisableVisualTasksEffect"/>
        /// </summary>
        ...
        internal DisableVisualTasksEffect(IGame game, GameManager gameManager, IGameStateTracker gameStateTracker, IExternalCommandFactory externalCommandFactory, ILogger logger)
        {
        ...
        /// We also cannot run this effect if Visual Tasks are already disabled
        /// </summary>
        ...
        public override bool IsReady()
        {
            if (gameStateTracker.HasGameState(game.Code) && gameStateTracker.HasStartingGameOptionsData(game.Code))
            {
                if (gameStateTracker.TryGetGameState(game.Code.Value, out CrowdControlGameState? gameState) && gameState.HasValue)
                {
                    ...
                    bool result = validTrackedState && validGameState && game.Options.VisualTasks;
        ...
        public override bool Start(Request context)
        {
            ...
            if (!game.Options.VisualTasks)
            {
                // If visual tasks are already disabled, we can't enable it again. 
                // This should get caught by IsReady but can't hurt to check twice
                logger.Write(Serilog.Events.LogEventLevel.Debug, $"{this.GetType().Name} Start returned False, VisualTasks are already disabled!");
                return false;
            }
            ...
            // Update the options to disable visual tasks
            ...
            updatePayload.GameOptions.VisualTasks = false;
