...
{
    /// <summary>
    /// An implementation of <see cref="Effect"/> which temporarily decreases the Impostor kill cooldown
    /// </summary>
    public class DecreaseKillCooldownEffect : Effect
    {
        public override string Code => "DecreaseKillCooldown";
        ...
        public override string[] Mutex => new string[] { "KillCooldown" };
        ...
        internal const float DecrementedKillCooldownScale = 0.75f; // TODO - Config

        internal DecreaseKillCooldownEffect(IGame game, GameManager gameManager, IGameStateTracker gameStateTracker, IExternalCommandFactory externalCommandFactory) 
            : this(game, gameManager, gameStateTracker, externalCommandFactory, Serilog.Log.ForContext<DecreaseKillCooldownEffect>())
        {

        }

        /// <summary>
        /// Internal ctor for creating a <see cref="DecreaseKillCooldownEffect"/>
        /// </summary>
        ...
        internal DecreaseKillCooldownEffect(IGame game, GameManager gameManager, IGameStateTracker gameStateTracker, IExternalCommandFactory externalCommandFactory, ILogger logger)
        {
        ...
        /// <remarks>
        /// We could run this in the lobby, but when this is done we need to be able to change the cooldown back to what it was when we started, settings can be changed anytime during the lobby
        ...
        public override bool Start(Request context)
        {
            ...
            // Update the options to include decreased kill cooldown
            ...
            updatePayload.GameOptions.KillCooldown = startingGameData.KillCooldown * DecrementedKillCooldownScale;
        ...
        public override bool Stop()
        {
            ...
            // Update the options to be whatever they are currently, but with the kill cooldown set back to what it was when the game started
            var updateCommand = externalCommandFactory.GetExternalCommand<UpdateGameExternalCommand>();
            var updatePayload = new UpdateGamePayload()
            {
                Code = game.Code,
            ...
            updatePayload.GameOptions.KillCooldown = startingGameData.KillCooldown;
            IExternalCommandResponse<Game> updateResponse = updateCommand.PerformCommand(updatePayload).Result;
