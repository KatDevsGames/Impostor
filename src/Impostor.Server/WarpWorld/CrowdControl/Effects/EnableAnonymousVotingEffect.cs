...
{
    /// <summary>
    /// An implementation of <see cref="Effect"/> which enabled Anonymous Voting in the game's options
    /// </summary>
    public class EnableAnonymousVotingEffect : Effect
    {
        public override string Code => "EnableAnonymousVoting";
        ...
        internal EnableAnonymousVotingEffect(IGame game, GameManager gameManager, IGameStateTracker gameStateTracker, IExternalCommandFactory externalCommandFactory)
            : this(game, gameManager, gameStateTracker, externalCommandFactory, Serilog.Log.ForContext<EnableAnonymousVotingEffect>())
        {

        }

        /// <summary>
        /// Internal ctor for creating a <see cref="EnableAnonymousVotingEffect"/>
        /// </summary>
        ...
        internal EnableAnonymousVotingEffect(IGame game, GameManager gameManager, IGameStateTracker gameStateTracker, IExternalCommandFactory externalCommandFactory, ILogger logger)
        {
        ...
        /// We also cannot run this effect if Anonymous Voting is already enabled
        /// </summary>
        ...
        public override bool IsReady()
        {
            if (gameStateTracker.HasGameState(game.Code) && gameStateTracker.HasStartingGameOptionsData(game.Code))
            {
                if (gameStateTracker.TryGetGameState(game.Code.Value, out CrowdControlGameState? gameState) && gameState.HasValue)
                {
                    ...
                    bool result = validTrackedState && validGameState && !game.Options.AnonymousVotes;
        ...
        public override bool Start(Request context)
        {
            ...
            if (game.Options.AnonymousVotes)
            {
                // If anonymous voting is already enabled, we can't enable it again. 
                // This should get caught by IsReady but can't hurt to check twice
                logger.Write(Serilog.Events.LogEventLevel.Debug, $"{this.GetType().Name} Start returned False, AnonymousVotes are already enabled!");
                return false;
            }
            ...
            // Update the options to include Anonymous Voting
            ...
            updatePayload.GameOptions.AnonymousVotes = true;
