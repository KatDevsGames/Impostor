...
{
    /// <summary>
    /// An implementation of <see cref="Effect"/> which temporarily increases kill distance
    /// </summary>
    public class IncreaseKillDistanceEffect : Effect
    {
        public override string Code => "IncreaseKillDistance";
        ...
        public const uint EffectID = 9;

        internal IncreaseKillDistanceEffect(IGame game, GameManager gameManager, IGameStateTracker gameStateTracker, IExternalCommandFactory externalCommandFactory)
            : this(game, gameManager, gameStateTracker, externalCommandFactory, Serilog.Log.ForContext<IncreaseKillDistanceEffect>())
        {

        }

        /// <summary>
        /// Internal ctor for creating a <see cref="IncreaseKillDistanceEffect"/>
        /// </summary>
        ...
        internal IncreaseKillDistanceEffect(IGame game, GameManager gameManager, IGameStateTracker gameStateTracker, IExternalCommandFactory externalCommandFactory, ILogger logger)
        {
        ...
        /// We also cannot run this effect if the Kill Distance is currently at it's maximum already
        /// </summary>
        ...
        public override bool IsReady()
        {
            if (gameStateTracker.HasGameState(game.Code) && gameStateTracker.HasStartingGameOptionsData(game.Code))
            {
                ...
                {
                    bool result = gameState.Value == CrowdControlGameState.InGame && game.GameState == GameStates.Started && game.Options.KillDistance != KillDistances.Long;
        ...
        public override bool Start(Request context)
        {
            ...
            if (game.Options.KillDistance == KillDistances.Long)
            {
                // If the Kill Distance is already as its maximum we can't increase anything
                // this should get caught in IsReady but to be safe
                logger.Write(Serilog.Events.LogEventLevel.Debug, $"{this.GetType().Name} Start returned False, KillDistance is already at Maximum");
                return false;
            }
            ...
            // Update the options to include increased kill distance
            ...
            updatePayload.GameOptions.KillDistance = KillDistances.Long;
