...
{
    /// <summary>
    /// An implementation of <see cref="Effect"/> which sabotages Comms on the current map
    /// </summary>
    public class SabotageCommsEffect : Effect
    {
        public override string Code => "SabotageComms";
        ...
        internal SabotageCommsEffect(IGame game, GameManager gameManager, IGameStateTracker gameStateTracker, IExternalCommandFactory externalCommandFactory)
            : this(game, gameManager, gameStateTracker, externalCommandFactory, Serilog.Log.ForContext<SabotageCommsEffect>())
        {

        }

        /// <summary>
        /// Internal ctor for creating a <see cref="SabotageCommsEffect"/>
        /// </summary>
        ...
        internal SabotageCommsEffect(IGame game, GameManager gameManager, IGameStateTracker gameStateTracker, IExternalCommandFactory externalCommandFactory, ILogger logger)
        {
        ...
        /// We can't run this game if the Comms system is already sabotaged
        /// </summary>
        ...
        public override bool Start(Request context)
        {
            ...
            if (t == null || !t.TryGetSabotageSystem(SystemTypes.Comms, out IActivatable act) || 
                (act.IsActive))
            {
                logger.Write(Serilog.Events.LogEventLevel.Debug, $"{this.GetType().Name} Start returned False, Comms is already sabotaged!");
                return false;
            }
            ...
            // Sabotaging Comms in Mira requires a countdown time to be sent to set/unset it as opposed to the simple boolean flag that Skeld/Polus/Airship required 
            if (game.Options.Map == MapTypes.MiraHQ)
            {
                setSabotagePayload.CountdownSystemState = 45;
            }
            else
            {
                setSabotagePayload.BooleanSystemState = true;
            }
