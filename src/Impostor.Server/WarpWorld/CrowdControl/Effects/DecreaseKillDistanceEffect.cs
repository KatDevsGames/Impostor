...
{
    /// <summary>
    /// An implementation of <see cref="Effect"/> which temporarily decreases kill distance
    /// </summary>
    public class DecreaseKillDistanceEffect : Effect
    {
        public override string Code => "DecreaseKillDistance";
        ...
        public override string[] Mutex => new string[] { "KillDistance" };
        ...
        public const uint EffectID = 10;

        internal DecreaseKillDistanceEffect(IGame game, GameManager gameManager, IGameStateTracker gameStateTracker, IExternalCommandFactory externalCommandFactory) 
            : this(game, gameManager, gameStateTracker, externalCommandFactory, Serilog.Log.ForContext<DecreaseKillDistanceEffect>())
        {

        }

        /// <summary>
        /// Internal ctor for creating a <see cref="DecreaseKillDistanceEffect"/>
        /// </summary>
        ...
        internal DecreaseKillDistanceEffect(IGame game, GameManager gameManager, IGameStateTracker gameStateTracker, IExternalCommandFactory externalCommandFactory, ILogger logger)
        {
        ...
        /// We also cannot run this effect if the Kill Distance is currently at it's minimum already
        /// </summary>
        /// <returns><c>true</c> if the effect can be ran, <c>false</c> if not</returns>
        /// <remarks>
        /// We could run this in the lobby, but when this is done we need to be able to change the distance back to what it was when we started, settings can be changed anytime during the lobby
        ...
        public override bool IsReady()
        {
            if (gameStateTracker.HasGameState(game.Code) && gameStateTracker.HasStartingGameOptionsData(game.Code))
            {
                ...
                {
                    bool result = gameState.Value == CrowdControlGameState.InGame && game.GameState == GameStates.Started && game.Options.KillDistance != KillDistances.Short;
        ...
        public override bool Start(Request context)
        {
            ...
            if (game.Options.KillDistance == KillDistances.Short)
            {
                // If the Kill Distance is already as its minimum we can't decrease anything
                // this should get caught in IsReady but to be safe
                logger.Write(Serilog.Events.LogEventLevel.Debug, $"{this.GetType().Name} Start returned False, KillDistance is already at minimum");
                return false;
            }
            ...
            // Update the options to include decreased kill distance
            ...
            updatePayload.GameOptions.KillDistance = KillDistances.Short;
        ...
        public override bool Stop()
        {
            ...
            // Update the options to be whatever they are currently, but with the kill distance set back to what it was when the game started
            var updateCommand = externalCommandFactory.GetExternalCommand<UpdateGameExternalCommand>();
            var updatePayload = new UpdateGamePayload()
            {
                Code = game.Code,
            ...
            updatePayload.GameOptions.KillDistance = startingGameData.KillDistance;
            IExternalCommandResponse<Game> updateResponse = updateCommand.PerformCommand(updatePayload).Result;
