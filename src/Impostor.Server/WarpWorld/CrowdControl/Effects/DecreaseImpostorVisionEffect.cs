...
{
    /// <summary>
    /// An implementation of <see cref="Effect"/> which temporarily decreases the impostor's vision
    /// </summary>
    public class DecreaseImpostorVisionEffect : Effect
    {
        public override string Code => "DecreaseImpostorVision";
        ...
        public override string[] Mutex => new string[] { "ImpostorVision" };
        ...
        internal const float DecrementedImpostorVisionScale = 0.5f; // TODO - Config

        /// <summary>
        /// Internal ctor for creating a <see cref="DecreaseImpostorVisionEffect"/>
        /// </summary>
        ...
        internal DecreaseImpostorVisionEffect(IGame game, GameManager gameManager, IGameStateTracker gameStateTracker, IExternalCommandFactory externalCommandFactory) 
            : this(game, gameManager, gameStateTracker, externalCommandFactory, Serilog.Log.ForContext<DecreaseImpostorVisionEffect>())
        {
        ...
        internal DecreaseImpostorVisionEffect(IGame game, GameManager gameManager, IGameStateTracker gameStateTracker, IExternalCommandFactory externalCommandFactory, ILogger logger)
        {
        ...
        /// <remarks>
        /// We could run this in the lobby, but when this is done we need to be able to change the vision back to what it was when we started, settings can be changed anytime during the lobby
        ...
        public override bool Start(Request context)
        {
            ...
            // Update the options to include decreased impostor vision
            ...
            updatePayload.GameOptions.ImpostorLightMod = startingGameData.ImpostorLightMod * DecrementedImpostorVisionScale;
        ...
        public override bool Stop()
        {
            ...
            // Update the options to be whatever they are currently, but with the impostor vision set back to what it was when the game started
            var updateCommand = externalCommandFactory.GetExternalCommand<UpdateGameExternalCommand>();
            var updatePayload = new UpdateGamePayload()
            {
                Code = game.Code,
            ...
            updatePayload.GameOptions.ImpostorLightMod = startingGameData.ImpostorLightMod;
            IExternalCommandResponse<Game> updateResponse = updateCommand.PerformCommand(updatePayload).Result;
